<?php
    ...
    function createLink($content,$href) {
        $element = new Element("a",$content);
        $element->addAttribute("href",$href);
        return $element;
    }

    function linkID($content,$id,$href = "#") {
        $element = createLink($content,$href);
    ...
    $div->addContent(linkID("<","arrowLeft")->getFullContent());

    for($i = 0; $i < ceil(count($contenu) / 4);$i++){
        $iInc = $i + 1; 
        $a = linkID($iInc,$iInc,$iInc);

        if($iInc == $id)
            $a->addAttribute('class',"active page");
        else
            $a->addAttribute("class","page");

        $div->addContent($a->getFullContent());
    }
